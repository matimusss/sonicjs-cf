      ...
      Promise.all([
        ...
        function compareById(list1, list2, idField) {
          ...
          function doesIdExist(array, id) {
            return array.some(item => item[idField] === id);
          }
        
          const onlyInList1 = [];  // Elementos que están en list1 pero no en list2
          const onlyInList2 = [];  // Elementos que están en list2 pero no en list1
        
          // Recorrer la lista 1
          list1.forEach(item1 => {
            const match = list2.find(item2 => item2[idField] === item1[idField]);
            if (!match) {
              // Si el id no está en list2
          ...
          list2.forEach(item2 => {
            if (!doesIdExist(list1, item2[idField])) {
              onlyInList2.push(item2);
            }
          });
        
          return {
            onlyInList1,  // Presentes solo en list1
            onlyInList2   // Presentes solo en list2
          };
        }
        ...
        const tagComparisons = compareById(productData.tags, obj1.tags, 'tag_id');
        ...
        const categoryComparisons = compareById(productData.categories, obj1.categories, 'cat_id');
console.log('Categories en productData pero no en obj1:', categoryComparisons.onlyInList1);
console.log('Categories en obj1 pero no en productData:', categoryComparisons.onlyInList2);


const supplierComparisons = compareById(productData.suppliers, obj1.suppliers, 'supplier_id');
console.log('Suppliers en productData pero no en obj1:', supplierComparisons.onlyInList1);
console.log('Suppliers en obj1 pero no en productData:', supplierComparisons.onlyInList2);
...
function compareCoupons(list1, list2) {
  ...
  // Verifica si un coupon_id existe en el array de cupones
  function doesCouponExist(couponsArray, couponId) {
    return couponsArray.some(coupon => coupon.coupon_id === couponId);
  }

  // Compara campos específicos en lugar de todos los campos
  function compareFields(coupon1, coupon2, fieldsToCheck) {
    return fieldsToCheck.every(field => coupon1[field] === coupon2[field]);
  }
  ...
  const changedInBoth = []; // Cupones presentes en ambos pero con diferencias en sus campos

  // Especifica los campos a comparar (por ejemplo, solo "coupon_id")
  const fieldsToCheck = ['coupon_id'];
  ...
  list1.forEach(coupon1 => {
    ...
    if (!match) {
      // Si el coupon_id no está en list2
      onlyInList1.push(coupon1);
    } else {
      // Si el ID está en ambos, comparamos solo los campos especificados
      if (!compareFields(coupon1, match, fieldsToCheck)) {
        changedInBoth.push({
  ...
  return {
    onlyInList1,  // Cupones presentes solo en list1
    onlyInList2,  // Cupones presentes solo en list2
    changedInBoth // Cupones presentes en ambos pero con cambios en campos seleccionados
  };
}
...
const couponComparisons = compareCoupons(productData.coupons, obj1.coupons);

// Imprimir los resultados de la comparación
console.log('Cupones en productData pero no en obj1:', couponComparisons.onlyInList1);
console.log('Cupones en obj1 pero no en productData:', couponComparisons.onlyInList2);
console.log('Cupones presentes en ambos pero cambiaron:', couponComparisons.changedInBoth);
